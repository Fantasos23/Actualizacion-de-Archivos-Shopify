...
def obtener_product_id(row):
    ...
    # ---------------------------------------------------------
    # 1. PASO 1: BÚSQUEDA DIRECTA POR SKU
    # ---------------------------------------------------------
    if v_serpi_plano:
        query_sku = """
        ...
        """
        # Buscar por SKU exacto o número plano
    ...
    # ---------------------------------------------------------
    # 2. PASO 2: BÚSQUEDA DIRECTA POR HANDLE
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 3. PASO 3: BÚSQUEDA DIFUSA POR TÍTULO CON SCAN DE METAFIELDS
    # ---------------------------------------------------------
    if v_desc:
        # Tomar las primeras 3 palabras clave del libro
        ...
        for edge in prods_t:
            ...
            # A) Verificar si el metafield serpi coincide en los datos devueltos
            ...
            # B) Verificar si los títulos limpios coinciden
...
def obtener_fecha_inicio_rango(horas):
    """Retorna la fecha inicial en formato 'yyyy-MM-dd' o 'dd-MM-yyyy' según el rango de horas."""
...
def crear_producto_en_shopify(item_serpi):
    """
    Crea un nuevo producto en Shopify de forma robusta en 2 pasos:
    1. Genera el Producto base y sus Metafields vía GraphQL.
    2. Actualiza SKU y Precio en la variante creada por defecto.
    """
    try:
        ...
        # 1. Preparar Metafields desde camposPersonalizados
        metafields_input = [{
            "namespace": "custom",
            "key": "serpi",
            "value": codigo,
        ...
        # Mapear otros campos personalizados con valor no vacío
        ...
        if not product_created:
            return None, [{"field": ["productCreate"], "message": f"Shopify no pudo procesar el producto. Respuesta raw: {res}"}]

        new_product_id = product_created.get("id")

        # 2. Asignar SKU y Precio a la variante recién generada
        ...
        # 3. Asignar Stock inicial si aplica
...
def inicializar_memoria_procesados():
    """Inicializa la lista de códigos SERPI procesados en la sesión."""
...
def registrar_producto_procesado(codigo_serpi):
    """Marca un producto como procesado exitosamente."""
...
def esta_procesado(codigo_serpi):
    """Verifica si un código SERPI ya fue procesado."""
    inicializar_memoria_procesados()
...
def actualizar_stock_shopify(product_id, nueva_cantidad):
    """
    Actualiza las existencias físicas de un producto en Shopify (SERPI -> Shopify)
    utilizando la API REST de Variantes para evitar bloqueos por permisos de ubicación.
    """
    try:
        # 1. Obtener la primera Variante del producto por GraphQL
        query_var = """
        ...
        # 2. Actualizar el stock y activar el seguimiento mediante API REST
        REST_URL = f"https://{RAW_SHOP_URL}/admin/api/{API_VERSION}"
        url_variant_rest = f"{REST_URL}/variants/{variant_numeric_id}.json"
        ...
        if res_rest.status_code in [200, 201]:
            return []  # Éxito total sin errores
        else:
...
# -------------------------------------------------------------
# 5. VENTANA FLOTANTE (MODAL/DIALOG)
# -------------------------------------------------------------
# -------------------------------------------------------------
# MODAL DE PREVISUALIZACIÓN UNIFICADO
# -------------------------------------------------------------
# -------------------------------------------------------------
# GESTIÓN DE CACHÉ / MEMORIA DE PRODUCTOS PROCESADOS
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# MODAL DE PREVISUALIZACIÓN UNIFICADO (AQUÍ VA ESTA FUNCIÓN)
# -------------------------------------------------------------
...
def mostrar_modal_previsualizacion_unificado(item_consolidado):
    ...
    with st.spinner("Comparando datos completos con Shopify..."):
        ...
        # -------------------------------------------------------------
        # ESCENARIO 1: EL PRODUCTO NO EXISTE EN SHOPIFY
        # -------------------------------------------------------------
        if not product_id:
            ...
            if st.button("✨ Crear este producto en Shopify", key="btn_create_modal_action"):
                with st.spinner("Creando producto y configurando variante/inventario en Shopify..."):
                    new_id, errs = crear_producto_en_shopify(item_consolidado)
                    if new_id:
                        registrar_producto_procesado(codigo_serpi)  # 👈 Guardar en memoria verde
                        st.balloons()
        ...
        # -------------------------------------------------------------
        # ESCENARIO 2: EL PRODUCTO SÍ EXISTE (ACTUALIZAR)
        # -------------------------------------------------------------
        else:
            query_detalles = """
...
# -------------------------------------------------------------
# PESTAÑA PRINCIPAL: SUPER SINCRONIZACIÓN UNIFICADA
# -------------------------------------------------------------
...
# Barra de estado de la memoria / caché
...
# -------------------------------------------------------------
# RENDERIZADO CON FILTROS Y ESTILO VERDE DE PROCESADOS
# -------------------------------------------------------------
if "cache_unificado" in st.session_state and st.session_state["cache_unificado"]:
    ...
    # Controles de visualización (Ocultar procesados)
    col_f1, col_f2 = st.columns([2, 1])
    with col_f1:
        ocultar_completados = st.checkbox("👁️‍🗨️ Ocultar productos ya procesados / actualizados", value=False)
    
    # Construcción de la tabla con indicador de estado visual
    resumen_tabla = []
    ...
    # Aplicar resaltado verde a las filas procesadas
    def resaltar_procesados(val):
        color = 'background-color: #d4edda; color: #155724; font-weight: bold;' if val == '✅ PROCESADO' else ''
        return color
    ...
    # Filtrar el desplegable según la preferencia del usuario
    ...
    if st.button("⚡ Sincronizar TODOS los datos pendientes en Shopify", key="btn_masivo_global"):
        ...
        # Sincronizar solo los que no han sido procesados
        ...
        if total_p == 0:
            ...
            for idx, item in enumerate(pendientes):
                ...
                if p_id:
                    ...
                    registrar_producto_procesado(cod_serpi)  # 👈 Se registra como procesado
                    exitos += 1
                else:
                    errores += 1
            ...
            st.rerun()
# -------------------------------------------------------------
# TAB: CARGA MANUAL VÍA EXCEL/CSV
# -------------------------------------------------------------
with tab_excel:
...
# -------------------------------------------------------------
# SECCIÓN: Subida Asistida de Portadas
# -------------------------------------------------------------
st.divider()
